...
const LayoutFeed: React.FC = () => {
  ...
  const [postagensComunidade, setPostagensComunidade] = useState<BeePostPropsExtended[]>([])
  const [postagensSeguidores, setPostagensSeguidores] = useState<BeePostPropsExtended[]>([])
  const [allPost, setAllPost] = useState<BeePostPropsExtended[]>([])
  const [postagensFiltroAvancado, setPostagensFiltroAvancado] = useState<BeePostPropsExtended[]>([])

  const [mostrarFiltradas, setMostrarFiltradas] = useState(false)
  const [carregandoFiltroAvancado, setCarregandoFiltroAvancado] = useState(false)
  const [filtrosAvancados, setFiltrosAvancados] = useState<FiltrosPostagem | null>(null)
  ...
  const categoriasParaTagsPostagem = (categoriasIds: number[]): IBeeTags[] => {
    ...
    const coresPorTipo: Record<"tec" | "mat" | "per", "magenta" | "orange" | "cyan"> = {
      tec: "magenta",
      mat: "orange",
      per: "cyan",
    }
  ...
  const aplicarFiltrosAvancados = async (filtros: FiltrosPostagem) => {
    ...
    const temFiltros = filtros.tecnologias.length > 0 || filtros.cursos.length > 0 || filtros.periodos.length > 0
    ...
    try {
      ...
      const filtradas = postagensParaFiltrar.filter((post) => {
        ...
        if (filtros.tecnologias.length > 0) {
          atendeTecnologia = filtros.tecnologias.some((tecId) => post.categorias?.includes(tecId))
        }

        if (filtros.cursos.length > 0) {
          atendeCurso = filtros.cursos.some((cursoId) => post.categorias?.includes(cursoId))
        }

        if (filtros.periodos.length > 0) {
          atendePeriodo = filtros.periodos.some((periodoId) => post.categorias?.includes(periodoId))
        }
              ...
              <div className="flex justify-end">
                <button
                  ...
                  className={`relative w-12 h-12 rounded-full transition-colors duration-200 flex items-center justify-center shadow-lg ${
                    totalFiltrosAtivos > 0 ? "bg-[#058B92] hover:bg-teal-700" : "bg-[#FCBD18] hover:bg-yellow-500"
                  }`}
