	...
	const carregarDados = async () => {
		...
		try {
			...
			try {
				...
				const arquivosDoRepo =
					arquivosResponse.data?.filter(
						(arquivo: any) => arquivo.repositorio === Number(id),
					) || [];

				const arquivosFormatados: FileItem[] = arquivosDoRepo.map(
					(arquivo: any) => ({
						id: arquivo.id,
						nome: arquivo.arquivo.split("/").pop() || "arquivo",
						arquivo: arquivo.arquivo,
						repositorio: Number(id),
					}),
				);
				...
				<div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8 text-center">
					<div className="text-red-500 mb-4">
						...
						<p className="text-gray-600 mt-2">
							{error ||
								"O repositório que você está procurando não existe ou foi removido."}
						</p>
					</div>
					<BeeButton
						label="Voltar"
