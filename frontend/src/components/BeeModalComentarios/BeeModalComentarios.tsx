...
import type { Comentario, ComentariosResponse } from "../../interfaces/Comentario"
...
function tempoDesde(data: string): string {
  ...
  const BeeModalComentarios: React.FC<IBeeModalComentarios> = ({ isOpen, onClose, post, onComentarioAdicionado }) => {
  ...
  const carregarComentarios = async () => {
    ...
    setLoading(true)
    try {
      const response: ComentariosResponse = await ComentarioService.getComentariosByPostagem(post.id)
  ...
  const enviarComentario = async () => {
    ...
    try {
      const comentarioCriado = await ComentarioService.criarComentario(post.id, novoComentario.trim())
            ...
            <div className="max-h-[70vh] overflow-y-auto">
              ...
              <div className="p-4 space-y-4">
                {loading ? (
                  ...
                  <div className="text-center py-8 text-gray-500">Seja o primeiro a comentar!</div>
                ) : (
                  comentarios.map((comentario) => (
                    <div key={comentario.id} className="flex gap-3 p-3 rounded-lg hover:bg-gray-50 transition-colors">
                      {/* Avatar do usuário */}
                      <img
                        src={
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="bg-gray-100 rounded-2xl px-4 py-2">
                          <div className="flex items-center mb-1">
                            <span className="font-semibold text-sm text-[#333333]">{comentario.usuario.nome}</span>
                            <span className="text-xs text-gray-500">@{comentario.usuario.username}</span>
                          </div>
                          <p className="text-sm text-[#333333] break-words">{comentario.conteudo}</p>
                        </div>
                        <div className="mt-1 px-2">
                          <span className="text-xs text-gray-500">{tempoDesde(comentario.dataPostagem)}</span>
                        </div>
                      </div>
                    </div>
                  ))
