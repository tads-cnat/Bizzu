  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between bg-white py-4 px-8 shadow-sm">
      <div className="flex items-center">
        <Link to={`/`}>
          <img src="/logo.png" alt="BIZZU Logo" className="w-32" />
        </Link>
      </div>
      {username === undefined ? (
        <BeeNotification
      ...
      {username != undefined ? (
        <BeeButton
          onClick={() => sair()}
          icone={<SignOut size={20} weight="bold" />}
          label="Sair"
          variante="negativo"
          className="ml-4"
        />
        ...
        <Link to={`/login/`}>
          <BeeButton icone={<SignIn size={20} weight="bold" />} label="Entrar" variante="aviso" className="ml-4" />
        </Link>
      )}
    </header>
  )
