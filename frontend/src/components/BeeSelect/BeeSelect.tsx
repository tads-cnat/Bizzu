...
const BeeSelect = ({ options, placeholder, icone: Icon, value, onChange, error }: IBeeSelect) => {
  const [selected, setSelected] = useState<any>(value || { label: placeholder, value: "" })
    ...
    <div>
      <Listbox value={selected} onChange={handleChange}>
        <div className="relative mt-2 w-fit inline-flex">
          <ListboxButton
            className="grid cursor-default grid-cols-1 rounded-lg bg-[#FFFFFF] py-1.5 pr-2 pl-3 text-left text-[#333333] outline-1 -outline-offset-1 outline-[#B0B0B0] focus:outline-2 focus:-outline-offset-2 focus:outline-[#333333] sm:text-sm/6"
            type="button"
          >
            <span className="col-start-1 row-start-1 flex items-center gap-3 pr-6">
              {Icon && <Icon size={20} weight="bold" style={{ marginRight: 8 }} />}
              <span className="block truncate">{selected?.label || placeholder}</span>
              <span className="block truncate cursor-pointer" onClick={selected?.value ? handleClear : undefined}>
                {iconSelect}
              </span>
            </span>
          </ListboxButton>
            ...
            {options.map((op) => (
              ...
              >
                <div className="flex items-center">
                  <span className="ml-3 block truncate font-normal group-data-selected:font-semibold">{op.label}</span>
                </div>
              </ListboxOption>
            ))}
          </ListboxOptions>
