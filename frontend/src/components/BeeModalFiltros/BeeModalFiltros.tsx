...
const BeeModalFiltros: React.FC<IBeeModalFiltros> = ({ isOpen, onClose, onAplicarFiltros, filtrosAtuais }) => {
  ...
  const renderizarCategorias = (tipo: "tec" | "mat" | "per", selecionadas: number[]) => {
    const categoriasDoTipo = categoriasParaFiltro(tipo)

    if (categoriasDoTipo.length === 0) {
      return <p className="text-gray-500 text-sm">Nenhuma categoria encontrada</p>
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      {/* Backdrop mais sutil */}
      <div className="absolute inset-0 bg-white bg-opacity-80" onClick={onClose} />
